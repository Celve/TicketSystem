 ...
 */
INDEX_TEMPLATE_ARGUMENTS
void BPLUSTREENTS_TYPE::InsertFromFile(const std::string &file_name, Transaction *transaction) {
  //   int64_t key;
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
void BPLUSTREENTS_TYPE::RemoveFromFile(const std::string &file_name, Transaction *transaction) {
  //   int64_t key;
  //   std::ifstream input(file_name);
  //   while (input) {
  // input >> key;
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
void BPLUSTREENTS_TYPE::ToGraph(BPlusTreePage *page, BufferPoolManager *bpm, std::ofstream &out) const {  // NOLINT
