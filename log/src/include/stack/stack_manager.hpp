 ...
 */
    ...
    STACK_TYPE::StackManager(const StackManager<T> &rhs) {
        ...
        sum_ = rhs.sum_;
        stack_io_.open(file_name_, std::ios::binary | std::ios::in | std::ios::out);
        /* the file is not exist */
        if (!stack_io_.is_open()) {
            /* create a new file */
            stack_io_.clear();
            stack_io_.open(file_name_, std::ios::binary | std::ios::trunc | std::ios::out);
            stack_io_.close();
            stack_io_.open(file_name_, std::ios::binary | std::ios::in | std::ios::out);
        } else {
            /* read exist data */
    ...
    STACK_TEMPLATE
    void STACK_TYPE::Init(const std::string &file_name) {
        int offset = 0;
        stack_io_.seekp(offset);
