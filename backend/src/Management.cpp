...
TrainManagement::TrainManagement() : cmp2(2), cmp3(3), cmp4(3), cmp5(2) {
  ...
  train_database =
      new BPlusTreeIndexNTS<String<32>, Train, StringComparator<32>>(
          "train_database", cmp1);
  station_database = new BPlusTreeIndexNTS<DualString<32, 32>, Station,
                                           DualStringComparator<32, 32>>(
      "station_database", cmp2);
  daytrain_database = new BPlusTreeIndexNTS<StringAny<32, int>, DayTrain,
                                            StringAnyComparator<32, int>>(
      "daytrain_database", cmp3);
  order_database = new BPlusTreeIndexNTS<StringAny<32, int>, Order,
                                         StringAnyComparator<32, int>>(
      "order_database", cmp4);
  pending_order_database = new BPlusTreeIndexNTS<StringIntInt<32>, PendingOrder,
                                                 StringIntIntComparator<32>>(
...
string TrainManagement::release_train(Command &line) {
  ...
  for (int i = 1; i <= target_train.station_num; ++i) {
    ...
    station_database->InsertEntry(
        DualString<32, 32>(target_train.stations[i], t_id), tp_station);
  }
...
string TrainManagement::query_ticket(Command &line) {
  ...
  if (type == "time")
    Sort(tickets, 1, cnt, time_cmp);
  else
    Sort(tickets, 1, cnt, cost_cmp);
  ...
  for (int i = 1; i <= cnt; ++i) {
    ...
    daytrain_database->SearchKey(
        StringAny<32, int>(tickets[i].s.train_ID, start_day.get_value()), &all);
  ...
  for (int i = 0; i < ans1.size(); ++i) { //枚举经过起点s1的不同车次
    ...
    for (int j = 0; j < ans2.size(); ++j) { //枚举经过终点t1的不同车次
      ...
      for (int k = 1; k < t1.index; ++k)
        ends[++cnt2] = std::make_pair(train2.stations[k], k);
      if (!cnt1 || !cnt2)
        continue;
      ...
      for (int i1 = 1, i2 = 1; i1 <= cnt1 && i2 <= cnt2;) {
        if (starts[i1].first < ends[i2].first)
          ...
          if (updated) { //如果更新答案，就保存结果
            ...
            daytrain_database->SearchKey(
                StringAny<32, int>(train1.train_ID, start_day1.get_value()),
                &f1);
            daytrain_database->SearchKey(
                StringAny<32, int>(train2.train_ID, start_day2.get_value()),
                &f2);
...
string TrainManagement::buy_ticket(Command &line, AccountManagement &accounts) {
  ...
  vector<DayTrain> ans2;
  daytrain_database->SearchKey(
      StringAny<32, int>(train_ID, start_day.get_value()), &ans2);
  DayTrain tp = ans2[0];
...
string TrainManagement::refund_ticket(Command &line,
  ...
  order_database->InsertEntry(StringAny<32, int>(user_name, orders[x].order_ID),
                              orders[x]);
  ...
  vector<DayTrain> ans;
  daytrain_database->SearchKey(
      StringAny<32, int>(refund_order.train_ID,
  ...
  pending_order_database->ScanKey(
      StringIntInt<32>(refund_order.train_ID,
                       refund_order.start_day.get_value(),
                       refund_order.order_ID),
      &ans2, tp_cmp2);
  ...
  for (int i = 1; i <= CNT; ++i) {
    ...
    if (tp_daytrain.query_seat(pending_orders[i].from,
      ...
      all.clear();
      order_database->SearchKey(StringAny<32, int>(pending_orders[i].user_name,
                                                   pending_orders[i].order_ID),
                                &all);
      success_order = all[0];
      success_order.status = success;
      order_database->InsertEntry(
          StringAny<32, int>(pending_orders[i].user_name,
                             pending_orders[i].order_ID),
          success_order);
    }
  }
