...
public:
  ...
  struct NODE {
    ...
    //默认构造函数
    NODE()
        : data(nullptr), left(nullptr), right(nullptr), fa(nullptr),
          color(RED){};

    NODE(const value_type &_data, COLOR c = RED, NODE *f = nullptr,
         NODE *lt = nullptr, NODE *rt = nullptr)
        : fa(f), left(lt), right(rt), color(c) {
      data = new value_type(_data);
    }

    NODE(const Key &_key, const T &_T, COLOR c = RED, NODE *f = nullptr,
         NODE *lt = nullptr, NODE *rt = nullptr)
        : fa(f), left(lt), right(rt), color(c) {
    ...
    NODE(const NODE &other)
        : fa(nullptr), left(nullptr), right(nullptr), color(other.color) {
  ...
  public:
    ...
    void insert(const value_type &x) {
      ...
      while (1) {
        ...
        if (t) {
          //直接修改颜色
          if (t->left && t->left->color == RED && t->right &&
              t->right->color == RED) {
    ...
    void remove(const Key &del) {
      ...
      if (root == nullptr)
        return;
      if (equal(root->data->first, del) && root->left == nullptr &&
          root->right == nullptr) {
        len--;
        delete root;
        root = nullptr;
    ...
    void remove_adjust(NODE *&p, NODE *&t, NODE *&t2, const Key &del) {
      //                if (t == nullptr || t2 == nullptr || p == nullptr)
      //                return;
      if (t->color == RED)
        return;
      if (t == root) {
  ...
  public:
    ...
    // Notice: you may add some code in here and class const_iterator and
    // namespace sjtu to implement toy_traits_test, this part is only for bonus.
    ...
    iterator &operator--() {
      if (id->mp.root == nullptr)
        throw invalid_iterator(); //判断map空
      if (p == id->mp.head || p == nullptr)
        throw invalid_iterator();
      if (p == id->mp.end_node)
        p = id->mp.back();
      else if (p->left) {
  ...
  public:
    ...
    const_iterator(NODE *_p, const map<Key, T, Compare> *_id)
        : p(_p), id(_id) {}
   ...
   */
  iterator end() { return iterator(mp.end_node, this); }

  const_iterator cend() const { return const_iterator(mp.end_node, this); }

  /**
   * checks whether the container is empty
   * return true if empty, otherwise false.
   */
  bool empty() const { return !(mp.len); }

  /**
   * returns the number of elements.
   */
  size_t size() const { return mp.len; }
